...
class DifferentialMultiHeadCrossAttention(nn.Module):
    ...
    def __init__(self, num_heads: int, embed_dim: int, d_model: int, dropout: float = 0.0, lambda_init: float = 0.8, layer_index: int = 0):
        ...
        self.rms_norms = [nn.RMSNorm(embed_dim) for _ in range(num_heads)]
    ...
    def forward(self, x: torch.Tensor, encoder_out: torch.Tensor) -> torch.Tensor:
        ...
        heads = [(1 - self.lamda_init) * self.rms_norms[i](head) for i, head in enumerate(heads)]
...
class DifferentialCrossAttentionHead(nn.Module):
    def __init__(self, 
        ...
        self.lambda_init = nn.Parameter(lambda_init - 0.6 * torch.exp(-0.3 * layer_index))
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x: torch.Tensor, encoder_out: torch.Tensor) -> torch.Tensor:
        ...
        q1, q2 = torch.chunk(self.W_q(x), dim=-1)

        # keys and values come from encoder output
        # B,T,C @ C,head_dim -> B,T,head_dim
        k1, k2 = torch.chunk(self.W_k(x), dim=-1)
...
class DifferentialMultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None) -> torch.Tensor:
        # TODO: RMS norm on each each; compute lambda same way as cross-attention
